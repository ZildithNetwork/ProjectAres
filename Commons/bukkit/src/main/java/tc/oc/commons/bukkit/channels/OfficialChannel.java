...
@Singleton
public class OfficialChannel extends SimpleChannel implements Commands {

    static final Setting SETTING = new SettingBuilder()
        .name("OfficialChat").alias("oc")
        .summary("Show confidential staff info")
        .type(new BooleanType())
        .defaultValue(true).get();

    public static final String PERM_NODE = "chat.official";
    ...
    @Inject OfficialChannel(ConsoleCommandSender console, OnlinePlayers players, SettingManagerProvider settings) {
    ...
    @Command(aliases = "o",
             desc = "Sends a message to the official chat (or sets the staff channel to your default channel).",
             max = -1,
             min = 0,
             anyFlags = true,
             usage = "[message...]")
    ...
    public void onOfficialChatCommand(final CommandContext arguments, final CommandSender sender) throws CommandException {
            ...
            if (sender.hasPermission(PERM_RECEIVE)) {
                if (sender instanceof Player) {
                    ...
                    if (!oldChannel.equals(this)) {
                        sender.sendMessage(org.bukkit.ChatColor.YELLOW + "Changed default channel to officials chat");
                    } else {
                        throw new CommandException("Official chat is already your default channel");
                    }
                } else {
                    throw new CommandUsageException("You must provide a message.", "/oc <message...>");
                }
            } else {
