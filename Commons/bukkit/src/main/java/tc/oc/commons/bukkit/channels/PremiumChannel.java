...
@Singleton
public class PremiumChannel extends SimpleChannel implements Commands {

    static final Setting SETTING = new SettingBuilder()
        .name("PremiumChat").alias("pc")
        .summary("Show premium chat")
        .type(new BooleanType())
        .defaultValue(true).get();

    public static final String PERM_NODE = "chat.premium";
    public static final String PERM_SEND = PERM_NODE + ".send";
    public static final String PERM_RECEIVE = PERM_NODE + ".receive";

    public static final String PREFIX = ChatColor.WHITE + "[" + ChatColor.GREEN + "PREMIUM" + ChatColor.WHITE + "]";
    ...
    @Inject PremiumChannel(ConsoleCommandSender console, OnlinePlayers players, SettingManagerProvider settings) {
    ...
    @Command(aliases = "pc",
             desc = "Sends a message to the premium chat (or sets the premium channel to your default channel).",
             max = -1,
             min = 0,
             anyFlags = true,
             usage = "[message...]")
    ...
    public void onPremiumChatCommand(final CommandContext arguments, final CommandSender sender) throws CommandException {
            ...
            if (sender.hasPermission(PERM_RECEIVE)) {
                if (sender instanceof Player) {
                    ...
                    if (!oldChannel.equals(this)) {
                        sender.sendMessage(org.bukkit.ChatColor.YELLOW + "Changed default channel to premium chat");
                    } else {
                        throw new CommandException("Premium chat is already your default channel");
                    }
                } else {
                    throw new CommandUsageException("You must provide a message.", "/pc <message...>");
                }
            } else {
