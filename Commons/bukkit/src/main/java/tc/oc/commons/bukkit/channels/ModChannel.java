...
@Singleton
public class ModChannel extends SimpleChannel implements Commands {

    static final Setting SETTING = new SettingBuilder()
        .name("ModChat").alias("mc")
        .summary("Show moderator chat")
        .type(new BooleanType())
        .defaultValue(true).get();

    public static final String PERM_NODE = "chat.mod";
    public static final String PERM_SEND = PERM_NODE + ".send";
    public static final String PERM_RECEIVE = PERM_NODE + ".receive";

    public static final String PREFIX = ChatColor.WHITE + "[" + ChatColor.RED + "S" + ChatColor.WHITE + "]";
    ...
    @Inject ModChannel(ConsoleCommandSender console, OnlinePlayers players, SettingManagerProvider settings) {
    ...
    public void onModChatCommand(final CommandContext arguments, final CommandSender sender) throws CommandException {
            ...
            if (sender.hasPermission(PERM_RECEIVE)) {
                if (sender instanceof Player) {
                    ...
                    if (!oldChannel.equals(this)) {
                        sender.sendMessage(org.bukkit.ChatColor.YELLOW + "Changed default channel to moderator chat");
                    } else {
                        throw new CommandException("Moderator chat is already your default channel");
                    }
                } else {
                    throw new CommandUsageException("You must provide a message.", "/mc <message...>");
                }
            } else {
